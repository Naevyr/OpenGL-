...
unsigned int VertexBuffer::addMesh(std::vector<unsigned char>& data) {
	...
	glBufferSubData(
		GL_ARRAY_BUFFER, m_offsets[0].size, data.size(), data.data()
	);
...
void VertexBuffer::removeMesh(unsigned int m_meshID) {
	...
	std::vector<unsigned char> buffer(ptr, ptr + m_offsets[0].size);
	...
	glBufferData(
		GL_ARRAY_BUFFER,
		m_offsets[0].size - m_offsets[m_meshID].size,
		nullptr,
		GL_DYNAMIC_DRAW
	);
	...
	glBindBuffer(GL_ARRAY_BUFFER, m_glID);
	glBufferSubData(
		GL_ARRAY_BUFFER, 0, m_offsets[0].size - m_offsets[m_meshID].size, ptr
	);
