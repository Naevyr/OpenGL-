...
ForwardPipeline::ForwardPipeline(std::shared_ptr<TextureManager> TextureManager)
...
void ForwardPipeline::render(RenderSpecifications& specs) {
	...
	glFramebufferTexture2D(
		...
		GL_DEPTH_ATTACHMENT,
		GL_TEXTURE_2D,
		specs.depthTexture.getID(),
		0
	);

	glViewport(
		0, 0, specs.colorTexture.getWidth(), specs.colorTexture.getHeight()
	);
...
void ForwardPipeline::SetTransformUniform(
...
void ForwardPipeline::SetLightUniform(
	std::vector<Light>& lights, Environment& environment
) {
	for (int i = 0; i < m_materials.size(); i++) {
		...
		material.SetUniform<glm::vec4>(
			...
			glm::vec4(
				environment.globalIllumination,
				environment.globalIlluminationIntensity
			)
		);
	}
}
...
unsigned int ForwardPipeline::LoadMaterial(MaterialDescription
materialDefinition, std::unordered_map<std::string, unsigned int>&
local_textures, std::vector<Texture>& global_textures) {



    auto material = Material("resources/shaders/standard.vert",
"resources/shaders/standard.frag"); std::string albedoPath =
materialDefinition.albedoPath; if(local_textures.find(albedoPath) ==
local_textures.end())
    {
        ...
        material.SetUniform<Texture&>("u_Albedo",global_textures[global_textures.size()
- 1]);
    }
    else
        material.SetUniform<Texture&>("u_Albedo",
global_textures[local_textures[albedoPath]]);
    ...
    if(local_textures.find(normalPath) == local_textures.end())
    {
        ...
        material.SetUniform<Texture&>("u_Normal",global_textures[global_textures.size()
- 1]);
    }
    else
        material.SetUniform<Texture&>("u_Normal",
global_textures[local_textures[normalPath]]);
