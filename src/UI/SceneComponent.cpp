...
void SceneComponent::Render(Scene& scene) {
	...
	for (int i = 0; i < lights.size(); i++) {
		...
		if (expand) m_displayedLights.insert(i);

		if (m_displayedLights.count(i) == 0) continue;

		bool keepOpen = true;
		LightComponent::Render(lights[i], &keepOpen);
		if (!keepOpen) m_displayedLights.erase(i);
	}

	for (int i = 0; i < meshes.size(); i++) {
		bool expand;
		CollapsedEntityComponent::Render("Mesh " + i, expand);
		if (expand) m_displayedMeshes.insert(i);

		if (m_displayedMeshes.count(i) == 0) continue;
		bool keepOpen = true;
		MeshComponent::Render(meshes[i], &keepOpen);
		if (!keepOpen) m_displayedMeshes.erase(i);
	}
	...
	for (auto light : m_displayedLights) {
		if (light < lights.size()) filteredLights.insert(light);
	};
	...
	for (auto mesh : m_displayedMeshes) {
		if (mesh < meshes.size()) filteredMeshes.insert(mesh);
	};
